...
这个代理合约比[第46讲](https://github.com/AmazingAng/WTFSolidity/blob/main/46_ProxyContract/readme.md)中的简单。在它的`fallback()`函数中使用`implementation.delegatecall(msg.data);`将调用委托给逻辑合约，并用`内联汇编`返回调用结果或回滚交易。
