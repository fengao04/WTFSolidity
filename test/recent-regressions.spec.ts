...
describe("Recent critical regression fixes", () => {
  it("allows ERC20 airdrops when the approved allowance exactly matches the batch total", async () => {
    ...
    const token = await deploy(
      compiled["33_Airdrop/Airdrop.sol:ERC20"],
      "MyToken",
      "MTK"
    );
    ...
    expect(
      (await token.allowance(owner.address, airdrop.address)).toString()
    ).to.equal("0");
  });
...
`;
    ...
    const proxy = await deploy(
      compiled["47_Upgrade/Upgrade.sol:SimpleUpgrade"],
      logic.address
    );
    const [owner] = await ethers.getSigners();
    const proxyAsLogic = new ethers.Contract(
      proxy.address,
      compiled["test/TestLogic.sol:TestLogic"].abi,
      owner
    );
